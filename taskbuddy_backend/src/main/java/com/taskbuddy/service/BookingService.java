...
import com.taskbuddy.exception.AuthenticationFailedException;
...
public class BookingService {
    ...
    @Transactional
    public BookingResponse createBookingWithPayment(BookingRequest request, String email) {
        // Get customer
        ...
        booking.setStatus(Status.PENDING_PAY); // New status
...
//    }
    ...
    public void deleteBooking(Long bookingId, String userEmail) {
        ...
        if (!booking.getMyCustomer().getId().equals(customer.getId())) {
            throw new AuthenticationFailedException("You can only delete your own bookings");
        }
    ...
    public BookingResponse cancelBooking(Long bookingId, String customerEmail) {
        ...
        // Process refund if booking was paid (PENDING or BOOKED status means payment was completed)
        if (booking.getStatus() == Status.PENDING || booking.getStatus() == Status.BOOKED) {
            try {
    ...
    public BookingResponse acceptBooking(Long bookingId, String workerEmail) {
        ...
        if (!booking.getMyWorker().getId().equals(worker.getId())) {
            throw new AuthenticationFailedException("You can only accept your own bookings");
        }
    ...
    public BookingResponse rejectBooking(Long bookingId, String workerEmail) {
        ...
        if (!booking.getMyWorker().getId().equals(worker.getId())) {
            throw new AuthenticationFailedException("You can only reject your own bookings");
        }
    ...
    public void handlePaymentFailure(Long bookingId) {
        ...
        if (booking.getStatus() == Status.PENDING_PAY) {
            booking.setStatus(Status.CANCELLED);
            bookingRepository.save(booking);
        }
    }
